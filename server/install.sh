...
Options:
  ...
  --hermes-api-key KEY    Bearer token sent to Hermes /v1/chat/completions.
                          Required if your Hermes instance enforces auth
                          (manifests as HTTP 401 'Invalid API key' in the app).
  --help, -h              Show this help.

Environment overrides:
  BRIDGE_PORT, HERMES_URL, HERMES_API_KEY, ASSUME_YES, INSTALL_SCRIPT_URL.
EOF
      exit 0
      ;;
  esac
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --hermes-api-key)
      [[ $# -ge 2 ]] || fail "--hermes-api-key requires a value"
      HERMES_API_KEY="$2"; shift 2;;
    --help|-h)
...
if [[ -z "$ASSUME_YES" ]]; then
  ...
  if [[ -z "$HERMES_API_KEY" ]]; then
    printf "Hermes API key (press Enter to skip if Hermes runs without auth): "
    read -r answer </dev/tty || true
    [[ -n "${answer:-}" ]] && HERMES_API_KEY="$answer"
  fi
...
if [[ -f "$CONFIG_FILE" ]]; then
  ...
  # Preserve an existing api_key unless the user explicitly passed a new one.
  if [[ -z "$HERMES_API_KEY" ]]; then
    HERMES_API_KEY=$(grep -E '^api_key *=' "$CONFIG_FILE" | head -n1 | sed -E 's/.*"([^"]*)".*/\1/' || true)
    [[ -n "$HERMES_API_KEY" ]] && ok "reusing existing Hermes API key from ${CONFIG_FILE}"
  fi
else
...
# Escape any double quotes in the api key so it doesn't break the TOML.
HERMES_API_KEY_ESC=${HERMES_API_KEY//\"/\\\"}

cat > "$CONFIG_FILE" <<EOF
# Generated by install.sh — re-runs preserve the token and the Hermes api_key.
[bridge]
host = "0.0.0.0"
port = ${BRIDGE_PORT}
...
${BOLD}╚══════════════════════════════════════════════════════════════╝${RESET}
  ...
  Bridge URL:  ${BOLD}ws://${HOST_IP}:${BRIDGE_PORT}/ws${RESET}
